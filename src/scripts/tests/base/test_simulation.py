...
#    simulation.set_tax_projection(rate = g, method="per_capita")
    ...
    pv =  cohorts['default'].aggregate_generation_present_value("tax")
...
def test_compute_gen_imbalance():
    ...
    simulation = Simulation()    
    simulation.discount_rate['default'] = 0
    simulation.growth_rate['default'] = 0
    simulation.cohorts['default'] = cohort
    simulation.create_present_values('tax')
    print simulation.aggregate_pv['default'].tail(20).to_string()
    print simulation.cohorts['default'].head(20).to_string()
...
def test_comparison():
    ...
    #Parameters of the two scenarii
    population_dataframe = create_testing_population_dataframe(year_start=2001, year_end=2061)
    profiles_dataframe = create_constant_profiles_dataframe(population_dataframe, tax=1)
    ...
    simulation = Simulation()  
    simulation.set_population(population_dataframe)
    simulation.set_population(population_dataframe, scenario='alternative')
    ...
    simulation.set_growth_rate(g_alt, scenario='alternative')
    simulation.set_discount_rate(r_alt, scenario='alternative') 
    simulation.set_population_growth_rate(n_alt, scenario='alternative')

    simulation.set_gov_wealth(net_gov_wealth)
    simulation.set_gov_wealth(net_gov_wealth_alt, scenario='alternative')
    simulation.set_gov_spendings(net_gov_spending, scenario='default')
    simulation.set_gov_spendings(net_gov_spending, scenario='alternative')
    print simulation.__dict__

    simulation.create_cohorts() ; 
    print "cohort created ----------------"
    print simulation.__dict__
    simulation.create_cohorts(scenario='alternative')
    simulation.set_tax_projection(method="per_capita", rate=g)


    simulation.create_present_values(typ='tax')
    simulation.create_present_values(typ='tax', scenario='alternative')
    print "valeurs actualisées crées"
    ipl_base = simulation.compute_ipl(typ='tax')
    ipl_alt = simulation.compute_ipl(typ='tax', scenario='alternative', precision=False)
    
    print ipl_base
    print ipl_alt
    assert ipl_base == -12170
