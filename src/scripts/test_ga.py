...
#    profiles = create_constant_profiles_dataframe(population_dataframe, tax = -1, subsidies = .5)
    ...
    cohorts_test.new_type(cohorts_test, 'taxes' )
    cohorts_test[(0,0,2001), 'taxes']=1
    # TODO: finish this test
    r = 0.00
    g = 0.00 
    cohorts_test.population_project(200, method = 'stable')
    cohorts_test.proj_tax(cohorts_test, rate = g, typ = None, method = 'per_capita')
...
def test_column_combination():
    year_start = 2001
    pop_dataframe = create_empty_dframe(year_start, 2061)
    print pop_dataframe
